...
const SubscriptionList: React.FC = () => {
  const [subscriptions, setSubscriptions] = useState<SubscribedRepo[]>([]);
  const [loading, setLoading] = useState(true);
  const apiUrl = `https://api.github.com/users/muleA/subscriptions`;
  ...
  useEffect(() => {
    const fetchSubscriptions = async () => {
      try {
        ...
        const data: SubscribedRepo[] = await response.json();
  ...
  return (
    <div className="max-w-2xl mx-auto p-6 bg-white shadow-lg rounded-lg dark:bg-gray-800 dark:text-gray-200">
      <h2 className="text-2xl font-bold mb-4">Subscribed Repositories</h2>
